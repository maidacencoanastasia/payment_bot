...
PRICES = [
    LabeledPrice(label='Ноутбук', amount=1900),
    LabeledPrice(label='Прочная упаковка', amount=1900)
]

SUPERSPEED_SHIPPING_OPTION = ShippingOption(
    id='superspeed',
    title='Супер быстрая!'
).add(LabeledPrice('Лично в руки!', 1900))
...
POST_SHIPPING_OPTION.add(LabeledPrice('Кортонная коробка', 1900))
POST_SHIPPING_OPTION.add(LabeledPrice('Срочное отправление!', 1900))

PICKUP_SHIPPING_OPTION = ShippingOption(
    id='pickup',
    title='Самовывоз'
)
PICKUP_SHIPPING_OPTION.add(LabeledPrice('Самовывоз в Бельцах', 1855))
...
async def shipping_process(shipping_query: ShippingQuery):
    ...
    if shipping_query.shipping_address.country_code == 'MD':
        shipping_options.append(POST_SHIPPING_OPTION)

        if shipping_query.shipping_address.city == 'Бельцы':
